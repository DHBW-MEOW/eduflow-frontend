...
const Calendar = ({ studygoals, exams }: CalendarProps) => {
  ...
  const generateCalendarDays = () => {
    ...
    for (let day = 0; day < daysInMonth; day++) {
      ...
      const dailyStudyGoals = studygoals.studygoals.filter(
        (goal: StudyGoalData) => {
    ...
    <div className="calendar-container">
      <div className="calendar-header">
        ...
        <div className="calendar-navbar">
          <button className="navbar-button" onClick={previousMonth}>
            &lt;&lt;
          </button>
          <span>
            {months[month]} {year}
          </span>
          <button className="navbar-button" onClick={nextMonth}>
            &gt;&gt;
          </button>
        </div>
      ...
      {/* Calendar Grid */}
      <div className="calendar-cell-grid">{generateCalendarDays()}</div>
    </div>
  );
};
