...
export const fetchStudyGoalData = async (
  ...
  try {
    const studygoals: StudyGoalData[] = await fetchFromBackend<
      { id: number; topic_id: number; deadline: string }[]
    >({
      method: "GET",
      endpoint: "data/study_goal",
    });

    const getTopics = await Promise.all(
    ...
    const topics: TopicData[] = getTopics.filter(
      (topic): topic is TopicData => topic !== null,
    );
    const topicMap = new Map<number, TopicData>();
    topics.forEach((topic) => topicMap.set(topic.id, topic));

    const getCourses = await Promise.all(
    ...
    const courses: CourseData[] = getCourses.filter(
      (course): course is CourseData => course !== null,
    );
